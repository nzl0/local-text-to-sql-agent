...
from app.catalog import CATALOG, file_for
...
def _instrument(agent):
    ...
    Neden `Agent.ask()`'e bir callback/event-emitter parametresi eklemek
    yerine metotları dışarıdan sarıyoruz: `Agent` ve `app.cli` SSE'den hiç
    haberdar olmasın istiyoruz. Bu sayede CLI aynı `Agent`'ı hiçbir
    instrumentation olmadan doğrudan çağırabiliyor; SSE'ye özgü kod tamamen
    bu dosyada izole kalıyor.
    """
...
# allow_origins=["*"]: risksiz, çünkü uygulama tamamen yerel/LAN içi çalışır,
# kimlik doğrulama çerezi ya da gizli bir token taşımaz — dışarı sızacak bir
# oturum bilgisi yok. Vite dev sunucusunun farklı porttan (5173) erişebilmesi
